...
/*   Updated: 2021/02/03 17:13:49 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_phong		ft_phong(t_ray ray, t_inter inter, t_vector l_pos)
{
	...
	ht = ft_sub(ft_scale(inter.normal,
		(2 * ft_dot(inter.normal, (l_direction)))), (l_direction));
	prod = ft_dot(ht, viewer_direction) / ft_get_norm(viewer_direction) *
		ft_get_norm(viewer_direction);
	if (ft_dot(ht, viewer_direction) > 0)
